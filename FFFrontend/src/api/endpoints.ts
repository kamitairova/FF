...
export const jobsApi = {
  listPublic: () => apiFetch<{ items: JobPost[] }>(`/jobs`, { method: "GET" }),
  getPublic: (id: number) => apiFetch<{ job: JobPost }>(`/jobs/${id}`, { method: "GET" }),

  listMyCompany: (token: string) => apiFetch<{ items: JobPost[] }>(`/jobs/my/list`, { method: "GET", token }),

  createJob: (
    token: string,
    body: { title: string; description: string; salaryFrom?: number; salaryTo?: number; city?: string }
  ) =>
  ...
  deleteJob: (token: string, id: number) => apiFetch<void>(`/jobs/${id}`, { method: "DELETE", token }),
};


export const adminApi = {
  dashboard: async (_token: string) => ({ stats: {} as any }),
  users: async (_token: string, _qs?: string) => ({ data: [] as any[], page: 1, pageSize: 20, total: 0 }),
  jobsModeration: async (_token: string, _qs?: string) => ({ data: [] as any[], page: 1, pageSize: 20, total: 0 }),
  taxonomy: async (_token: string) => ({ items: [] as any[] }),
  setJobStatus: async (_token: string, _jobId: number, _status: any) => ({ ok: true }),
};
